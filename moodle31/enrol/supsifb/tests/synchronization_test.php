...
class enrol_supsifb_synchronization_testcase extends advanced_testcase {
    ...
    protected function setUp() {
        ...
        $this->supsifb_plugin = enrol_get_plugin('supsifb');
        ...
        $this->enrol_id = $this->supsifb_plugin->enrol_students_first_time($this->students_usernames,
        ...
        // situations
        \enrol_supsifb\ws\factory::inject_object($this->fake_ws);
    }
    ...
    public function test_synchronize_plugin_enrolment_without_changes() {
        ...
        foreach ($this->students_usernames as $username) {
            $fake_unique_id[] = new \enrol_supsifb\ws\student($username, $username, $username);
        }

        // inject the fake data in the web service
        $this->fake_ws->inject_students($this->fake_module_id, $fake_unique_id);

        // execute the sync and check that it succeds
        $this->assertTrue($this->supsifb_plugin->synchronize_plugin_enrolment($this->enrol_id));
        ...
        $this->assertInstanceOf('\enrol_supsifb\event\plugin_enrolment_synchronization_started', $start_event);

        // check that the second event is the end event
        $end_event = $events[1];
        $this->assertInstanceOf('\enrol_supsifb\event\plugin_enrolment_synchronization_completed', $end_event);
    }
    ...
    public function test_synchronize_plugin_enrolment_ws_creation_failure() {
        ...
        \enrol_supsifb\ws\factory::inject(function($wsdl, $key) {
            throw new \enrol_supsifb\ws\ws_creation_failed_exception('FAKE_EXCEPTION');
        });

        // execute the sync and check that it fails
        $this->assertFalse($this->supsifb_plugin->synchronize_plugin_enrolment($this->enrol_id));
        ...
        $this->assertInstanceOf('\enrol_supsifb\event\ws_creation_failed', $ws_creation_failure_event);

        // check that the third event is the failure event
        $failure_event = $events[2];
        $this->assertInstanceOf('\enrol_supsifb\event\plugin_enrolment_synchronization_failed', $failure_event);
    }
    ...
    public function test_synchronize_plugin_enrolment_ws_remote_call_failure() {
        ...
        $this->fake_ws->fail_on('get_students_enrolled', function ($module_id) {
            throw new \enrol_supsifb\ws\remote_call_failed_exception('FAKE_EXCEPTION');
        }); 
        ...
        $this->assertInstanceOf('\enrol_supsifb\event\ws_remote_call_failed', $remote_call_failure_event);
    ...
    public function test_synchronize_plugin_enrolment_zero_students_returned_failure() {
        ...
        $this->fake_ws->inject_students($this->fake_module_id, array());
