 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_usi_plugin extends enrol_plugin {
    ...
    public function get_newinstance_link($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context)
            or !has_capability('enrol/usi:config', $context)) {
    
            return NULL;
        }

        return new moodle_url('/enrol/usi/users_sync.php', array('id'=>$courseid));
    }
    ...
    public function get_action_icons(stdClass $instance) {
        ...
        // check the capabilities
        if (!has_capability('enrol/usi:config', $context)) {
            return array();
        }
        ...
        // create the sync URL
        $sync_link = new moodle_url("/enrol/usi/course_sync.php", $parameters);
     ...
     * @param string the USI module identifier
     * @param string the USI module name
     *
    ...
    public function enrol_students_first_time($students_ids, $course_id, $target_module_id, $target_module_name) {
        ...
        try {
            ...
            \enrol_usi\event\enrol_students_first_time_started::create(array(
                'context' => $context,
                'other' => array('usi_module_name' => $target_module_name,
                                 'usi_module_id' => $target_module_id)
            ))->trigger();
            ...
            // trigger an event to signal this enrolment action
            \enrol_usi\event\enrol_students_first_time_completed::create(array(
                'objectid' => $enrol_id,
                'context' => $context,
                'other' => array('usi_module_name' => $target_module_name,
                                 'usi_module_id' => $target_module_id)
        ...
        \enrol_usi\event\enrol_students_first_time_failed::create(array(
            'objectid' => $enrol_id,
            'context' => $context,
            'other' => array('usi_module_name' => $target_module_name,
                             'usi_module_id' => $target_module_id)
            ...
            foreach ($students_ids as $current_id) {
                ...
                if ($DB->record_exists('user', array('username' => $current_id))) {
                    ...
                    if (!is_enrolled($context, $moodle_user)) {
                        ...
                        $default_role = get_config('enrol_usi', 'roleid');
     ...
     * looking at what the USI Web Service returns.
     *
    ...
    public function synchronize_plugin_enrolment($enrol_id) {
        ...
        \enrol_usi\event\plugin_enrolment_synchronization_started::create(array(
            'context' => context_course::instance($course_id),
            'courseid' => $course_id
        ))->trigger();

        // extract the USI module identifier from the enrolment instance custom fields
        $usi_module_id = $enrol_instance->customchar1;
        $usi_module_name = $enrol_instance->customchar2;

        /*
         * USI students retrieval
         */

        // try to get an instance of the USI Web Service
        $usi_ws = null;
        try {
            ...
            // interrupted
            \enrol_usi\event\plugin_enrolment_synchronization_failed::create(array(
                'context' => context_course::instance($course_id),
                'courseid' => $course_id,
                'other' => array('error_message' => 'Cannot create the USI Web Service instance.')
            ))->trigger();
        ...
        $usi_students = null;
        try {
            $usi_students = $usi_ws->get_unique_ids($usi_module_id);
            ...
            // trigger an event to signal that the synchronization has been 
            // interrupted
            $error_message  = 'Cannot retrieve the students unique IDs from the USI Web Service ';
        ...
        if (count($usi_students) == 0) {
            // trigger an event to signal that weird situation
            \enrol_usi\event\plugin_enrolment_synchronization_failed::create(array(
                'context' => context_course::instance($course_id),
                'courseid' => $course_id,
                'other' => array('error_message' => 'The number of USI students is zero, this is really strange.')
            ))->trigger();

            return false; 
        }

        // normalize the usi students array (it will only contain the IDs as strings)
        $usi_students = array_map(function($student) { return $student->get_id(); }, $usi_students);
        ...
        /*
         * Students comparison (USI vs Moodle)
         */

        // find the students to enrol
        $students_to_enrol = $this->compute_students_to_enrol($usi_students, $moodle_students);

        // find the students to unenrol
        $students_to_unenrol = $this->compute_students_to_unenrol($usi_students, $moodle_students);
        ...
        if ($errors_happened) {
            ...
            // successfully
            \enrol_usi\event\plugin_enrolment_synchronization_completed::create(array(
                'context' => context_course::instance($course_id),
                'courseid' => $course_id
            ))->trigger();
    ...
    /*
     * Synchronize all the courses that used this plugin for enrolling USI students.
     */
    public function synchronize_all_plugin_enrolments() {
        global $DB;

        // trigger an event to signal that the synchronization is started
        \enrol_usi\event\plugin_enrolment_mass_synchronization_started::create(array(
            'context' => context_system::instance()
        ))->trigger();

        // restrict the query only on the plugin enrolment instances
        $conditions = array('enrol' => 'usi');
        ...
        try {
            ...
            // trigger an event to signal that the synchronization has been 
            // interrupted
            \enrol_usi\event\plugin_enrolment_mass_synchronization_failed::create(array(
                'context' => context_system::instance(),
        ...
        // trigger an event to signal that the synchronization completed 
        // successfully
        \enrol_usi\event\plugin_enrolment_mass_synchronization_completed::create(array(
            'context' => context_system::instance()
        ))->trigger();
     ...
     *
     * @param array the uniqueIDs of the USI students
     * @param array the usernames of the Moodle students 
     *
     * @return array the users that need to be enrolled
     */
    public function compute_students_to_enrol($usi_students, $moodle_students) {
        return array_diff($usi_students, $moodle_students);
    }
     ...
     */
    public function compute_students_to_unenrol($usi_students, $moodle_students) {
        return array_diff($moodle_students, $usi_students);
    }
}
