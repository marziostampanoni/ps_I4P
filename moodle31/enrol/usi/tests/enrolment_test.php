...
class enrol_usi_enrolment_testcase extends advanced_testcase {
    ...
    public function test_enrol_students_first_time() {
        ...
        $usi_plugin = enrol_get_plugin('usi');
        ...
        $usi_module_id = 'USI_COURSE';
        $usi_module_name = 'USI Course In Random Science';

        // redirect the events
        $sink = $this->redirectEvents();

        // enrol the users in the courses
        $eid = $usi_plugin->enrol_students_first_time($students_usernames, $course->id, $usi_module_id, $usi_module_name);
        ...
        $this->assertInstanceOf('\enrol_usi\event\enrol_students_first_time_started', $start_event);

        // check that the last event is the end event
        $end_event = $events[count($events)-1];
        $this->assertInstanceOf('\enrol_usi\event\enrol_students_first_time_completed', $end_event);
    ...
    public function test_enrol_students() {
        ...
        // enrol the new users
        $usi_plugin->enrol_students($other_students_usernames, $eid);
    ...
    public function test_unenrol_students() {
        ...
        $usi_plugin->unenrol_students($students_usernames, $eid);
    ...
    public function test_get_enrolled_users_with_plugin() {
         ...
         * the plugin.
         */
        $usernames = $usi_plugin->get_enrolled_users_with_plugin($course->id, $eid); 
    ...
    public function test_compute_students_to_enrol_and_unenrol() {
        ...
        foreach (range(0,9) as $i) {
            $usi_users[] = $this->getDataGenerator()->create_user();
        }
        ...
        // create an array for the users' usernames
        $usi_usernames = array();
        foreach ($usi_users as $user) {
            $usi_usernames[] = $user->username;
        }
        ...
        $users_to_enrol = $usi_plugin->compute_students_to_enrol($usi_usernames, $moodle_usernames);
        $this->assertEquals(count($usi_users), count($users_to_enrol));

        // compute the users to unenrol 
        $users_to_unenrol = $usi_plugin->compute_students_to_unenrol($usi_usernames, $moodle_usernames);
