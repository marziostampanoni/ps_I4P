...
foreach ($req as $idReq => $request) {
    $cond = array('id_requestmanager_richiesta' => $idReq, 'tipo_richiesta' => 'Cancellare', 'stato_richiesta' => STATO_RICHIESTA_DA_GESTIRE);
...
$form = new FormCancella(new moodle_url($CFG->wwwroot . '/local/requestmanager/delete.php'), array('courses' => $courses), 'post', '', array('id' => 'deleteForm'));
...
if ($fromform = $form->get_data()) {
    ...
    $request->saveToDB();
    redirect(new moodle_url($CFG->wwwroot . '/local/requestmanager/manage.php'));
}
$filter->display();
$form->display();
