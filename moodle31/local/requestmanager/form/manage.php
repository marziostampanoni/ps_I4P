                    ...
                    foreach ($requests as $request) {
                        ...
                        foreach ($request->corsi_richiesti as $corso) {
                            if ($corso->stato_richiesta == STATO_RICHIESTA_DA_GESTIRE || !is_siteadmin($USER->id)) {
                                $form->addElement('html', '<tr bgcolor="#d3d3d3"><th>' . get_string('manage_tablehead_type', 'local_requestmanager') . ' / ' . get_string('resume_tablehead_actions', 'local_requestmanager') . '</th><th colspan="3"  style="border-right: solid 1px lightgray;">' . get_string('resume_tablehead_cat', 'local_requestmanager') . '</th></tr>');
                                $action = 'save';
                                ...
                                $form->addElement('html', '<span style="top: -10px;">' . local_requestmanager\CEUtil::tipoRichiesta($corso->tipo_richiesta)  . '</span>');
                                $form->addElement('html', '<br><hr>');
                                $form->addElement('html', '<a onclick="actionOnCourse(' . $corso->id . ', \'' . $action . '\')">' . get_string($string, 'local_requestmanager') . '</a>');
                                ...
                                $form->addElement('html', '<br><a onclick="actionOnCourse(' . $corso->id . ', \'' . $action . '\')">' . get_string($string, 'local_requestmanager') . '</a>');
                                ...
                                $form->addElement('html', '<tr  bgcolor="#d3d3d3" style="border-right: solid 1px lightgray;"><th colspan="2" style="width: 70%">' . get_string('resume_tablehead_title', 'local_requestmanager') . '</th><th colspan="2">' . get_string('manage_tablehead_shortname', 'local_requestmanager') . '</th><tr style="background-color: ' . $color . '">');
                                $form->addElement('html', '<tr style="background-color: ' . $color . '">');
                                ...
                                $form->addElement('html', '<tr  bgcolor="#d3d3d3"><th style="width: 33%">' . $teachersBtn . ' - ' . get_string('resume_tablehead_teacher', 'local_requestmanager') . '</th><th style="width: 33%">' . $editingteachersBtn . ' - ' . get_string('resume_tablehead_editingteacher', 'local_requestmanager') . '</th><th style="width: 33%; border-right: solid 1px lightgray; vertical-align:middle;">' . get_string('resume_tablehead_note', 'local_requestmanager') . '</th></tr><tr style="background-color: ' . $color . '">');
