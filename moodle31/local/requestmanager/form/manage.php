                ...
                if (is_array($requests)) {
                    ...
                    foreach ($requests as $request) {
                        ...
                        foreach ($request->corsi_richiesti as $corso) {
                            ...
                            $form->addElement('html', '<br><select ' . ($corso->tipo_richiesta == TIPO_RICHIESTA_CANCELLARE || $corso->stato_richiesta == STATO_RICHIESTA_FATTO ? 'disabled' : '') . ' style="width:100%;" name="categoria-' . $corso->id . '" id="categoria-' . $corso->id . '">');
                            ...
                            $form->addElement('html', '</td>');
                            $form->addElement('html', '<td rowspan="2" style="border: none; padding: 0px 10px;"><label>' . get_string('resume_tablehead_note', 'local_requestmanager') . '</label><textarea ' . ($corso->tipo_richiesta == TIPO_RICHIESTA_CANCELLARE || $corso->stato_richiesta == STATO_RICHIESTA_FATTO ? 'disabled' : '') . ' name="note-' . $corso->id . '" style="resize: none; display:table-cell; width:95%;height: 70%;">' . $corso->note . '</textarea></td>');
                            $form->addElement('html', '</tr>');
                            $form->addElement('html', '<tr>');
