...
if (isset($_GET['cancel']) && $_GET['cancel'] > 0) {
    ...
    $corso->saveToDB();
    $courdeDeleted = delete_course($corso->id);
    $DB->delete_records('course', array('id'=>$corso->id_mdl_course));
    $DB->delete_records('course_sections', array('course'=>$corso->id_mdl_course));
    $DB->delete_records('course_format_options', array('courseid'=>$corso->id_mdl_course));
...
if (isset($_GET['save']) && $_GET['save'] > 0) {
    ...
    foreach (explode('_', $_GET['save']) as $item) {
        ...
        $corso->loadFromDB();
        $course4moodle = new stdClass();
        $course4moodle->category = $corso->getIdMdlCourseCategories();
        $course4moodle->fullname = $corso->getTitolo();
        $course4moodle->shortname = $corso->getShortname();
        $course = create_course($course4moodle);
